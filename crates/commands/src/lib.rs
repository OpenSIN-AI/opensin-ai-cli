...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect OpenSIN config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded OpenSIN instruction memory files",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter OPENSIN.md for this repo",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
    ...
    SlashCommandSpec {
        ...
        summary: "Manage OpenSIN Code plugins",
        argument_hint: Some(
...
pub fn render_slash_command_help() -> String {
    let mut lines = vec![
        ...
        "  [resume] = also available via opensin --resume SESSION.json".to_string(),
    ];
...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::ProjectOpenSIN => "Project (.opensin)",
            Self::UserCodexHome => "User ($CODEX_HOME)",
            Self::UserCodex => "User (~/.codex)",
            Self::UserOpenSIN => "User (~/.opensin)",
        }
    }
}
...
pub fn handle_commit_push_pr_slash_command(
    ...
    let body_path = write_temp_text_file("opensin-pr-body", "md", request.pr_body.trim())?;
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectOpenSIN,
            ancestor.join(".opensin").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserOpenSIN,
            home.join(".opensin").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOpenSIN,
            ancestor.join(".opensin").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOpenSIN,
            ancestor.join(".opensin").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOpenSIN,
            home.join(".opensin").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOpenSIN,
            home.join(".opensin").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       opensin agents".to_string(),
        "  Sources          .codex/agents, .opensin/agents, $CODEX_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       opensin skills".to_string(),
        "  Sources          .codex/skills, .opensin/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
mod tests {
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "OpenSIN Tests"]);
        run_command(&root, "git", &["config", "user.email", "opensin@example.com"]);
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".opensin-plugin")).expect("manifest dir");
        fs::write(
            root.join(".opensin-plugin").join("plugin.json"),
            format!(
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".opensin").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectOpenSIN,
        ...
        assert!(report.contains("Project (.opensin):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       opensin agents"));
