...
mod tests {
    ...
    fn computes_cost_summary_lines() {
        ...
        let lines = usage.summary_lines_for_model("usage", Some("opensin-ai-sonnet-4-6"));
        assert!(lines[0].contains("estimated_cost=$54.6750"));
        assert!(lines[0].contains("model=opensin-ai-sonnet-4-6"));
    ...
    fn supports_model_specific_pricing() {
        ...
        let haiku = pricing_for_model("opensin-ai-haiku-4-5-20251213").expect("haiku pricing");
        let opus = pricing_for_model("opensin-ai-opus-4-6").expect("opus pricing");
