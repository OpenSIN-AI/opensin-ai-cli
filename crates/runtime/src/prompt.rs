...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("OPENSIN.md"),
            dir.join("OPENSIN.local.md"),
            dir.join(".opensin").join("OPENSIN.md"),
            dir.join(".opensin").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "OpenSIN instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# OpenSIN instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No OpenSIN Code settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".opensin")).expect("nested opensin dir");
        fs::write(root.join("OPENSIN.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("OPENSIN.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".opensin")).expect("apps opensin dir");
        fs::write(root.join("apps").join("OPENSIN.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".opensin").join("instructions.md"),
            "apps dot opensin instructions",
        )
        .expect("write apps dot opensin instructions");
        fs::write(nested.join(".opensin").join("OPENSIN.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".opensin").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("OPENSIN.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("OPENSIN.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.opensin/OPENSIN.md")),
            "OPENSIN.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("OPENSIN.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? OPENSIN.md"));
    ...
    #[test]
    fn load_system_prompt_reads_opensin_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".opensin")).expect("opensin dir");
        fs::write(root.join("OPENSIN.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".opensin").join("settings.json"),
        ...
        let original_opensin_home = std::env::var("OPENSIN_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("OPENSIN_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_opensin_home {
            std::env::set_var("OPENSIN_CONFIG_HOME", value);
        } else {
            std::env::remove_var("OPENSIN_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_opensin_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".opensin")).expect("opensin dir");
        fs::write(root.join("OPENSIN.md"), "Project rules").expect("write OPENSIN.md");
        fs::write(
            root.join(".opensin").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# OpenSIN instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_opensin_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".opensin")).expect("nested opensin dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".opensin/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/OPENSIN.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# OpenSIN instructions"));
