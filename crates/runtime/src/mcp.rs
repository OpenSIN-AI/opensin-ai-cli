...
const OPENSINAI_SERVER_PREFIX: &str = "opensin-ai.ai ";
...
pub fn normalize_name_for_mcp(name: &str) -> String {
    ...
    if name.starts_with(OPENSINAI_SERVER_PREFIX) {
...
pub fn scoped_mcp_config_hash(config: &ScopedMcpServerConfig) -> String {
    let rendered = match &config.config {
        ...
        McpServerConfig::ManagedProxy(proxy) => {
            format!("opensin-aiai-proxy|{}|{}", proxy.url, proxy.id)
        }
    };
...
mod tests {
    ...
    fn normalizes_server_names_for_mcp_tooling() {
        ...
        assert_eq!(
            normalize_name_for_mcp("opensin-ai.ai Example   Server!!"),
            "opensin-ai_ai_Example_Server"
        );
        assert_eq!(
            mcp_tool_name("opensin-ai.ai Example Server", "weather tool"),
            "mcp__opensin-ai_ai_Example_Server__weather_tool"
        );
    }
