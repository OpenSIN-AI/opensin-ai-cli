...
pub const OPENSIN_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".opensin.json"),
            |parent| parent.join(".opensin.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".opensin.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".opensin").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".opensin").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("OPENSIN_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".opensin")))
        .unwrap_or_else(|| PathBuf::from(".opensin"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".opensin.json");
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "opensin-aiai-proxy" => Ok(McpServerConfig::ManagedProxy(McpManagedProxyServerConfig {
            url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".opensin");
    ...
    #[test]
    fn loads_and_merges_opensin_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".opensin");
        fs::create_dir_all(cwd.join(".opensin")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".opensin.json"),
        ...
        fs::write(
            cwd.join(".opensin.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".opensin").join("settings.json"),
            r#"{"env":{"C":"3"},"hooks":{"PostToolUse":["project"]},"mcpServers":{"project":{"command":"uvx","args":["project"]}}}"#,
        )
        .expect("write project settings");
        fs::write(
            cwd.join(".opensin").join("settings.local.json"),
        ...
        assert_eq!(OPENSIN_SETTINGS_SCHEMA_NAME, "SettingsSchema");
