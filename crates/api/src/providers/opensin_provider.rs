...
fn resolve_saved_oauth_token_set(
    ...
    let client = OpenSINApiClient::from_auth(AuthSource::None).with_base_url(read_base_url());
...
impl Provider for OpenSINApiClient {
...
mod tests {
    ...
    use super::{
        ...
        resolve_startup_auth_source, AuthSource, OpenSINApiClient, OAuthTokenSet,
    };
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::set_var("OPENSIN_CONFIG_HOME", &config_home);
        std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
        std::env::remove_var("ANTHROPIC_API_KEY");
        save_oauth_credentials(&runtime::OAuthTokenSet {
    ...
    fn backoff_doubles_until_maximum() {
        let client = OpenSINApiClient::new("test-key").with_retry_policy(
            3,
