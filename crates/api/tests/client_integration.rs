...
async fn send_message_posts_json_and_parses_response() {
    ...
    let body = concat!(
        ...
        "\"type\":\"message\",",
        "\"role\":\"assistant\",",
        "\"content\":[{\"type\":\"text\",\"text\":\"Hello from OpenSIN\"}],",
        "\"model\":\"opensin-ai-sonnet-4-6\",",
    ...
    assert_eq!(
        ...
        vec![OutputContentBlock::Text {
            text: "Hello from OpenSIN".to_string(),
        }]
    );
...
async fn stream_message_parses_sse_events_with_tool_use() {
    ...
    let sse = concat!(
        "event: message_start\n",
        "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_stream\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[],\"model\":\"opensin-ai-sonnet-4-6\",\"stop_reason\":null,\"stop_sequence\":null,\"usage\":{\"input_tokens\":8,\"output_tokens\":0}}}\n\n",
    ...
    let server = spawn_server(
        ...
        vec![
            ...
            http_response(
                "200 OK",
                "application/json",
                "{\"id\":\"msg_retry\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Recovered\"}],\"model\":\"opensin-ai-sonnet-4-6\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":2}}",
            ),
        ],
    )
    .await;
...
async fn provider_client_dispatches_api_requests() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"msg_provider\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Dispatched\"}],\"model\":\"opensin-ai-sonnet-4-6\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":2}}",
        )],
    )
    .await;

    let client = ProviderClient::from_model_with_default_auth(
        "opensin-ai-sonnet-4-6",
        Some(AuthSource::ApiKey("test-key".to_string())),
    )
    .expect("api provider client should be constructed");
    let client = match client {
        ProviderClient::OpenSINApi(client) => {
            ProviderClient::OpenSINApi(client.with_base_url(server.base_url()))
        }
...
async fn live_stream_smoke_test() {
    ...
    let mut stream = client
        .stream_message(&MessageRequest {
            model: std::env::var("OPENSIN_MODEL").unwrap_or_else(|_| "opensin-ai-sonnet-4-6".to_string()),
            max_tokens: 32,
...
fn sample_request(stream: bool) -> MessageRequest {
    MessageRequest {
        model: "opensin-ai-sonnet-4-6".to_string(),
        max_tokens: 64,
