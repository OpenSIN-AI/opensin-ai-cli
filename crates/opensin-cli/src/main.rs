...
use api::{
    resolve_startup_auth_source, AuthSource, OpenSINApiClient, ContentBlockDelta, InputContentBlock,
...
const DEFAULT_MODEL: &str = "opensin-ai-opus-4-6";
...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             opensin --help".to_string());
    lines.join("\n")
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // OpenSIN Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // OpenSIN Code compat: --print makes output non-interactive
...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `opensin` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `opensin --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn resolve_model_alias(model: &str) -> &str {
    match model {
        "opus" => "opensin-ai-opus-4-6",
        "sonnet" => "opensin-ai-sonnet-4-6",
        "haiku" => "opensin-ai-haiku-4-5-20251213",
        _ => model,
    }
}
...
fn default_permission_mode() -> PermissionMode {
    env::var("OPENSIN_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::opensin_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.opensin.dev/oauth/authorize"),
        token_url: String::from("https://platform.opensin.dev/v1/oauth/token"),
        callback_port: None,
        manual_redirect_url: None,
        scopes: vec![
            String::from("user:profile"),
            String::from("user:inference"),
            String::from("user:sessions:opensin_code"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting OpenSIN OAuth login...");
    ...
    let client = OpenSINApiClient::from_auth(AuthSource::None).with_base_url(api::read_base_url());
    ...
    println!("OpenSIN OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("OpenSIN OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "OpenSIN OAuth login failed. You can close this window."
    } else {
        "OpenSIN OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_opensin_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("OPENSIN.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;45m🦞 OpenSIN Code\x1b[0m"
                } else {
                    "OpenSIN Code"
                },
                if color {
        ...
        if !has_opensin_md {
            lines.push(
                "  First run        /init scaffolds OPENSIN.md, .opensin.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("opensin-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("opensin-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("opensin-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".opensin").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No OPENSIN instruction files discovered in the current directory ancestry.".to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_opensin_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_opensin_md()?);
    Ok(())
}
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: OpenSINApiClient::from_auth(resolve_cli_auth_source()?)
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "OpenSIN Code CLI v{VERSION}")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  opensin                                  Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  opensin \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  opensin prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  opensin --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  opensin dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  opensin bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  opensin agents                           List configured agents"
    )?;
    writeln!(
        out,
        "  opensin skills                           List installed skills"
    )?;
    writeln!(out, "  opensin system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(
        out,
        "  opensin login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  opensin logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
        "  opensin init                             Scaffold OPENSIN.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
    ...
    writeln!(out, "  opensin --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  opensin --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  opensin --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  opensin --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  opensin agents")?;
    writeln!(out, "  opensin /skills")?;
    writeln!(out, "  opensin login")?;
    writeln!(out, "  opensin init")?;
    Ok(())
}
...
mod tests {
    ...
    fn resolves_model_aliases_in_args() {
        ...
        assert_eq!(
            ...
            CliAction::Prompt {
                prompt: "explain this".to_string(),
                model: "opensin-ai-opus-4-6".to_string(),
                output_format: CliOutputFormat::Text,
                allowed_tools: None,
                permission_mode: PermissionMode::DangerFullAccess,
            }
    ...
    fn resolves_known_model_aliases() {
        assert_eq!(resolve_model_alias("opus"), "opensin-ai-opus-4-6");
        assert_eq!(resolve_model_alias("sonnet"), "opensin-ai-sonnet-4-6");
        assert_eq!(resolve_model_alias("haiku"), "opensin-ai-haiku-4-5-20251213");
    ...
    fn shared_help_uses_resume_annotation_copy() {
        ...
        assert!(help.contains("available via opensin --resume SESSION.json"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("opensin init"));
        assert!(help.contains("opensin agents"));
        assert!(help.contains("opensin skills"));
        assert!(help.contains("opensin /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_opensin_md(std::path::Path::new("."));
        assert!(rendered.contains("# OPENSIN.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/opensin-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p opensin-cli"}"#)
                .contains("cargo test -p opensin-cli")
        );
        assert_eq!(
