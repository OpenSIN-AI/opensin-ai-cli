...
const STARTER_OPENSIN_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# OpenSIN Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".opensin/settings.local.json", ".opensin/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let opensin_dir = cwd.join(".opensin");
    artifacts.push(InitArtifact {
        name: ".opensin/",
        status: ensure_dir(&opensin_dir)?,
    });

    let opensin_json = cwd.join(".opensin.json");
    artifacts.push(InitArtifact {
        name: ".opensin.json",
        status: write_file_if_missing(&opensin_json, STARTER_OPENSIN_JSON)?,
    });
    ...
    let opensin_md = cwd.join("OPENSIN.md");
    let content = render_init_opensin_md(cwd);
    artifacts.push(InitArtifact {
        name: "OPENSIN.md",
        status: write_file_if_missing(&opensin_md, &content)?,
    });
...
pub(crate) fn render_init_opensin_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# OPENSIN.md".to_string(),
        String::new(),
        "This file provides guidance to OpenSIN Code (opensincode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.opensin.json`; reserve `.opensin/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `OPENSIN.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_opensin_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("opensin-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".opensin/           created"));
        assert!(rendered.contains(".opensin.json       created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("OPENSIN.md          created"));
        assert!(root.join(".opensin").is_dir());
        assert!(root.join(".opensin.json").is_file());
        assert!(root.join("OPENSIN.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".opensin.json")).expect("read opensin json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".opensin/settings.local.json"));
        assert!(gitignore.contains(".opensin/sessions/"));
        let opensin_md = fs::read_to_string(root.join("OPENSIN.md")).expect("read opensin md");
        assert!(opensin_md.contains("Languages: Rust."));
        assert!(opensin_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("OPENSIN.md"), "custom guidance\n").expect("write existing opensin md");
        fs::write(root.join(".gitignore"), ".opensin/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("OPENSIN.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".opensin/           skipped (already exists)"));
        assert!(second_rendered.contains(".opensin.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("OPENSIN.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("OPENSIN.md")).expect("read existing opensin md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".opensin/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".opensin/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_opensin_md(Path::new(&root));
