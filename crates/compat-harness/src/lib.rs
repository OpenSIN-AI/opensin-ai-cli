...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("OPENSIN_CODE_UPSTREAM") {
        candidates.push(PathBuf::from(explicit));
    }

    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("opensin-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("opensin-code"));
    candidates.push(primary_repo_root.join("vendor").join("opensin-code"));
...
pub fn extract_bootstrap_plan(source: &str) -> BootstrapPlan {
    ...
    if source.contains("--opensin-ai-in-chrome-mcp") {
