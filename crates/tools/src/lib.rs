...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set OpenSIN Code settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("opensin-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("OPENSIN_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OPENSIN_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".opensin-todos.json"))
}
...
const DEFAULT_AGENT_MODEL: &str = "opensin-ai-opus-4-6";
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("opensin-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OPENSIN_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".opensin-agents"));
    }
    Ok(cwd.join(".opensin-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "opensinguide" | "opensinguideagent" | "guide" => String::from("opensin-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".opensin").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("OPENSIN_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".opensin"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("opensin-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("OPENSIN_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("OPENSIN_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("OPENSIN_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("OPENSIN_TODO_STORE");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("OPENSIN_AGENT_STORE", &dir);
        ...
        std::env::remove_var("OPENSIN_AGENT_STORE");
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("opensin-ai-sonnet-4-6".to_string()),
            },
            |job| {
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".opensin")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".opensin")).expect("cwd dir");
        std::fs::write(
            home.join(".opensin").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        .expect("write global settings");

        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("OPENSIN_CONFIG_HOME").ok();
        let original_dir = std::env::current_dir().expect("cwd");
        ...
        match original_config_home {
            Some(value) => std::env::set_var("OPENSIN_CONFIG_HOME", value),
            None => std::env::remove_var("OPENSIN_CONFIG_HOME"),
        }
